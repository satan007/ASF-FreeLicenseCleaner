...
- **Релиз с готовым zip** — запушьте тег вида `X.Y.Z`, **БЕЗ** буквы `v`
  ...
  **Важно про префикс `v`:** `IGitHubPluginUpdates` в самой ASF делает
  ...
  Release. Версия сейчас `0.0.11` — подтверждено, что плагин реально
  сканирует и удаляет лицензии на живом боте, но пока это была лишь
  ограниченная обкатка, так что до `1.0.0` ещё рано.

  **Важно про папку установки:** `FreeLicenseCleaner.zip` (с `0.0.11`)
  содержит файлы плагина **прямо в корне архива, без обёртки** — так и
  должно быть, это соответствует тому, как ASF сама применяет обновление
  плагина (распаковывает архив как есть внутрь уже существующей папки
  плагина, ничего не разворачивая). Поэтому при первой ручной установке
  создайте подпапку `<ASF>/plugins/FreeLicenseCleaner/` **сами** и
  распакуйте архив **в неё** — а не в `<ASF>/plugins/` напрямую и не в
  какую-то другую вложенность. Ниже — два реальных бага, к которым в
  проде привели отступления от этого правила (в первом уже безопасно —
  плагин сам это обнаруживает и отключает себе автообновление, во
  втором — нет).
...
прямо в `<ASF>/plugins/`, а не в `<ASF>/plugins/FreeLicenseCleaner/` (до
`0.0.11` архив релиза содержал обёртку `FreeLicenseCleaner/`, и при
неаккуратной распаковке "сюда" файлы могли лечь плоско), то ASF при
следующем автообновлении плагина посчитает СВОЕЙ папкой сам `plugins/`
целиком (`assemblyDirectory` в ASF — это просто папка, где физически
лежит .dll плагина). Дальше срабатывает тот же самый механизм переноса в
`_old`, что и раньше с `data/` — только теперь под раздачу попадают **все
остальные установленные плагины**: их подпапки целиком переезжают в
`plugins/_old/<ИмяПлагина>/`, а на месте остаётся пустая
`plugins/<ИмяПлагина>/`. Сам FreeLicenseCleaner при этом не пропадает и
даже выглядит нормально — новые файлы из релиза (архив до `0.0.11` как
раз содержал обёртку `FreeLicenseCleaner/`) ложатся туда же, куда были до
...
### До версии `0.0.11` каждое обновление создавало ещё одну вложенную папку плагина

Независимый от бага выше, но по одной и той же причине: до `0.0.11`
архив релиза (`FreeLicenseCleaner.zip`) был устроен с обёрткой — внутри
архива всё лежало не в корне, а в подпапке `FreeLicenseCleaner/`. ASF же
при автообновлении плагина распаковывает архив **как есть** внутрь уже
существующей папки плагина (`<ASF>/plugins/FreeLicenseCleaner/`) и
переносит файлы, сохраняя их путь внутри архива. С обёрткой в архиве это
означало, что новые файлы каждый раз оказывались на уровень глубже:
`plugins/FreeLicenseCleaner/` → `plugins/FreeLicenseCleaner/FreeLicenseCleaner/`
→ `plugins/FreeLicenseCleaner/FreeLicenseCleaner/FreeLicenseCleaner/` и
так далее на каждое следующее успешное автообновление — ASF находит .dll
рекурсивно на любой глубине, поэтому плагин продолжал работать и ничего
не выглядело сломанным, кроме растущей вложенности папок (и отдельной
папки `data/` на каждом уровне — на тех версиях, где `data/` ещё лежала
рядом с самим плагином, до фикса из раздела про потерю истории выше).

**Если у вас так** — заранее остановите ASF (или хотя бы не давайте ему
проверять обновления), затем:

1. Найдите **самый глубокий** уровень вложенности — там лежат реальные
   `FreeLicenseCleaner.dll` и остальные файлы текущей рабочей версии.
2. Сравните `data/*.json`, если они есть на разных уровнях (по дате
   изменения/размеру) — самый свежий, скорее всего, на самом глубоком
   уровне. Если ещё не переносили его в `<ASF>/config/plugins/FreeLicenseCleanerPlugin/`
   (см. раздел про потерю истории) — сделайте это сейчас.
3. Перенесите файлы с самого глубокого уровня прямо в
   `<ASF>/plugins/FreeLicenseCleaner/` (один уровень, без вложенности) и
   удалите опустевшие промежуточные подпапки `FreeLicenseCleaner/FreeLicenseCleaner/...`.
4. Перезапустите ASF, проверьте `flc status`.

**С `0.0.11`** архив релиза больше не содержит обёртку — файлы лежат
прямо в его корне, так что новое автообновление ляжет ровно туда же,
где плагин уже находится, без углубления. Это никак не лечит уже
случившуюся вложенность — только останавливает её дальнейший рост.
